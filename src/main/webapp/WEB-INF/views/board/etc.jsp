...
<body class="is-preload">
		...
		<div class="row">
			...
			<c:forEach var="musemob" items="${musemobs}">
				<div class="slides_content_left col-6 col-12-small">
					...
					<a href="etc/${musemob.musemobid }/chapmchoice" class="chapmchoice button small">응원하기</a>
				...
				<div class="slides_content_right col-6 col-12-small">
					...
					<a href="etc/${musemob.musemobid }/challchoice" class="challchoice button small">응원하기</a>
				</div>
			</c:forEach>
					...
					<section>
						<div class="content">
							...
							<ul class="actions">
								...
								<li><a href="etc/${list.postid}/support" class="button small">후원하기</a>
	...
	<div id="myModal" class="modal">
		...
		<div class="modal-content">
			...
			<form action="etc/newchallenger" method="post">
				<div>
	...
	<div class="page-move-btn">
		<ul>
			<c:if test="${pageMaker.prev }">
				<li><a
					href='<c:url value="/board/etc?page=${pageMaker.startPage-1}"/>'>PREV</a></li>
				...
				<c:choose>
					...
					<c:when test="${idx==page}">
						<li><a class="currentPage"
							href='<c:url value="/board/etc?page=${idx }"/>'>${idx }</a></li>
					</c:when>
					<c:otherwise>
						<li><a href='<c:url value="/board/etc?page=${idx }"/>'>${idx }</a></li>
					</c:otherwise>
			...
			<c:if test="${pageMaker.next && pageMaker.endPage >0 }">
				<li><a
					href='<c:url value="/board/etc?page=${pageMaker.endPage+1 }"/>'>NEXT</a></li>
			</c:if>
		</ul>
	</div>
