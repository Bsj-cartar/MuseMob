...
<body class="is-preload">
				...
				<c:forEach var="musemob" items="${musemobs}">
					<div class="slides_content_left col-6 col-12-small">
						<div class="slides_content">
							...
							<a href="post/${musemob.championpost.postid}"> <img
								src="http://i.ytimg.com/vi/${musemob.championpost.postvideourl}/0.jpg" />
							</a>

						</div>

						<a href="vocal/${musemob.musemobid }/chapmchoice"
							class="chapmchoice button small">응원하기</a>
					...
					<div class="slides_content_right col-6 col-12-small">
						<div class="slides_content">
							...
							<a href="post/${musemob.challengerpost.postid}"> <img
								src="http://i.ytimg.com/vi/${musemob.challengerpost.postvideourl}/0.jpg" />
							</a>

						</div>

						<a href="vocal/${musemob.musemobid }/challchoice"
							class="challchoice button small">응원하기</a>
					</div>
				</c:forEach>
	...
	<section class="wrapper">
		<div class="inner">
			<header class="special">
				...
				<p>
					대결하고 싶은 동영상에 대결 신청을 눌러보세요. <br> 최근에 공유한 동영상 세 개 중 하나를 선택해서 경쟁할
					수 있습니다. <br> 게시판 상단에 노출 되어 다른 회원들에게 응원받을 수 있습니다. <br> 더
					많이 응원을 받은 사람이 포인트를 몽땅 가져가겠죠 ?
				</p>
				<a href="postupload" class="button primary large">업로드</a>
			</header>
			<c:choose>
				<c:when test="${pageMaker.totalCount <= 0 }">
					<div class="highlights_error">
						<h2>등록된 게시물이 없습니다.</h2>
					</div>
				</c:when>
				<c:otherwise>
								...
								<div class="content">
									<header>
										<div class="video">
											...
											<a href="post/${list.postid }"> <img
												src="http://i.ytimg.com/vi/${list.postvideourl}/mqdefault.jpg" />
											</a>
									...
									<ul class="actions">
										...
										<li><a href="vocal/${list.postid}/support"
											class="button small">후원하기</a>
										<li><a href="#Redirect"
											onClick="getPostId(<c:out value="${list.postid}"/>)"
											class="button primary small">대결 신청</a></li>
					...
					<div class="page-move-btn">
						<ul>
							<c:if test="${pageMaker.prev }">
								<li><a
									href='<c:url value="/board/vocal?page=${pageMaker.startPage-1}"/>'>PREV</a></li>
								...
								<c:choose>
									...
									<c:when test="${idx==page}">
										<li><a class="currentPage"
											href='<c:url value="/board/vocal?page=${idx }"/>'>${idx }</a></li>
									</c:when>
									<c:otherwise>
										<li><a href='<c:url value="/board/vocal?page=${idx }"/>'>${idx }</a></li>
									</c:otherwise>
							...
							<c:if test="${pageMaker.next && pageMaker.endPage >0 }">
								<li><a
									href='<c:url value="/board/vocal?page=${pageMaker.endPage+1 }"/>'>NEXT</a></li>
							</c:if>
						</ul>
					</div>
				</c:otherwise>
	...
	<div id="myModal" class="modal">
		...
		<div class="modal-content">
			...
			<form action="vocal/newchallenger" method="post">
				<div>
					<c:forEach var="recentlyPost" items="${recentlyPostInst}">
						...
						<label for="radio-${recentlyPost.postid}"> <iframe
								src="http://www.youtube.com/embed/${recentlyPost.postvideourl}?controls=2showinfo=0"
